...
站点由 GitHub Actions 构建并部署到 GitHub Pages，包含下方全部 6 本书的完整正文与统一的侧边目录，方便连续、跨书阅读。
...
- **`custom.css` / `custom.js`**：纸质书阅读主题、字号调节与目录排版优化。

书籍正文始终以上表仓库为唯一来源，本仓库通过软链接引用原始文件，**不重复存储、不复制内容**。构建产物由 GitHub Actions 上传至 GitHub Pages。
...
git clone https://github.com/Lularible/safety-book-iso26262.git
...
- **本仓库的构建脚本与配置**（`book.toml`、`generate_summary.py`、`setup_symlinks.py`、`custom.css`、`custom.js` 等）：仅为生成聚合阅读站点所用的构建工具，与书籍内容的许可证无关，不代表对书籍正文享有任何权利。
